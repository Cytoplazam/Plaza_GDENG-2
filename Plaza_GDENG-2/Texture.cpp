...
Texture::Texture(const wchar_t* full_path): Resource(full_path)
{
	...
	if (SUCCEEDED(res))
	{
		res = DirectX::CreateTexture(GraphicsEngine::get()->getD3Ddevice(), image_data.GetImages(), image_data.GetImageCount(), image_data.GetMetadata(), &m_texture);
		...
		GraphicsEngine::get()->getD3Ddevice()->CreateShaderResourceView(m_texture, &desc, &m_shader_res_view);
	}
	else
	{
