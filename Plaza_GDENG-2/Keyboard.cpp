...
InputSystem Keyboard::ReadKey()
{
	if (this->keyBuffer.empty()) //If no keys to be read?
	{
		return InputSystem(); //return empty keyboard event
	}
	else
	{
		InputSystem e = this->keyBuffer.front(); //Get first Keyboard Event from queue
...
void Keyboard::OnKeyPressed(const unsigned char key)
{
	...
	this->keyBuffer.push(InputSystem(InputSystem::EventType::Press, key));
}

void Keyboard::OnKeyReleased(const unsigned char key)
{
	this->keyStates[key] = false;
	this->keyBuffer.push(InputSystem(InputSystem::EventType::Release, key));
}
